# APE — Play Store launch checklist (AccessibilityService)

APE uses the AccessibilityService API to detect and block short-form feeds. Google
...
- **Prominent disclosure + consent**: a full-screen disclosure now appears *before*
  APE sends you to enable Accessibility (in onboarding and on the "You" screen). It
...
- **Keep-alive**: a "Keep APE running" card lets the user exempt APE from battery
...
1. **Permissions Declaration Form → AccessibilityService section.** Declare it and state,
   ...
   - *Functionality*: "APE helps users reduce doomscrolling. It uses AccessibilityService
...
3. **Privacy policy URL** (required). Since APE is fully offline, it can be one short
...
4. **Data safety form**: select "No data collected" and "No data shared" (true for APE).
